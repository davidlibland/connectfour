...
def test_batch_winner():
    bgs = MutableBatchGameState(batch_size=3, turn=PlayState.X, num_cols=7, num_rows=7)
    plays1 = [4, 4, 3, 3, 2]
    plays2 = [4, 4, 3, 3, 1]
    plays3 = [1, 5, 1, 0, 1]
    bgs = reduce(MutableBatchGameState.play_at, zip(plays1, plays2, plays3), bgs)
...
def test_batch_next_actions():
    bgs = MutableBatchGameState(batch_size=2, turn=PlayState.X, num_cols=7, num_rows=3)
