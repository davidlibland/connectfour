...
def test_batch_winner():
    bgs = BatchGameState(batch_size=3, turn=PlayState.X, num_cols=7, num_rows=7)
    plays1 = [4,4,3,3,2]
    plays2 = [4,4,3,3,1]
    plays3 = [1,5,1,0,1]
...
def test_batch_next_actions():
    bgs = BatchGameState(batch_size=2, turn=PlayState.X, num_cols=7, num_rows=3)
    ...
    assert next_actions[0].tolist() == [True, True, True, False, False, True, True]
    assert next_actions[1].tolist() == [True, True, True, True, True, True, True]
...
def test_gaussian_neg_log_likelihood(eps=1e-3):
    ...
    assert torch.all(torch.less_equal(torch.abs(expected_log_prob - actual_log_prob), eps))
